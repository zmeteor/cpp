...
int DBI_mysql::db_connect(const char* host, const char* database, const char* usr, const char* pwd, int port)
{
	...
	char value = 1;
	mysql_options(_conn, MYSQL_OPT_RECONNECT, &value); //MYSQL_OPT_RECONNECT 断开重连
...
bool DBI_mysql::db_autocommit(bool flag)  //设置成非自动提交
{
	clearerr();
...
int DBI_mysql::db_begin(void) //执行sql操作之前的操作
{
	clearerr();
    //设置成非自动提交
...
void DBI_mysql::db_free_result(void)
{
	clearerr();
...
int DBI_mysql::db_prepare_sql(dbi_prepare_t* q, const char* sql)
{
    ...
    //初始化一个状态，准备字符串查询指向的sql语句并返回一个对象供mysql_stmt_prepare使用
